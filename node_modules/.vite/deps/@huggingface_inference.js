...
var HFInferenceZeroShotClassificationTask = class extends HFInferenceTask {
  async getResponse(response) {
    if (Array.isArray(response) && response.every((x) => Array.isArray(x.labels) && x.labels.every((_label) => typeof _label === "string") && Array.isArray(x.scores) && x.scores.every((_score) => typeof _score === "number") && typeof x.sequence === "string")) {
      return response;
    }
...
${snippet}`;
        ...
        if (snippet.includes(placeholder)) {
          snippet = replaceAccessTokenPlaceholder(opts == null ? void 0 : opts.directRequest, placeholder, snippet, language, provider);
        }
...
function replaceAccessTokenPlaceholder(directRequest, placeholder, snippet, language, provider) {
  ...
  const accessTokenEnvVar = useHfToken ? "HF_TOKEN" : provider.toUpperCase().replace("-", "_") + "_API_KEY";
  if (language === "sh") {
